...
function PipelinePage() {
  ...
  async function moveProspect(prospectId: string, stageId: string) {
    ...
    if (!prospect || prospect.stage_id === stageId) return;
    ...
    const { error: activityError } = await supabase.from("activities").insert({ user_id: userId, prospect_id: prospectId, type: "stage_alterada", description: `Etapa alterada de ${stages.find((stage) => stage.id === prospect.stage_id)?.name ?? "sem etapa"} para ${target?.name ?? "sem etapa"}.` });
    if (activityError) toast.error("Prospect movido, mas não foi possível registrar a atividade.");
    invalidate(["prospects", "activities"]);
  }
  async function addStage() { const name = newStage.trim(); if (!name) return; try { const userId = await currentUserId(); const color = STAGE_COLORS[stages.length % STAGE_COLORS.length]!; const { error } = await supabase.from("pipeline_stages").insert({ user_id: userId, name, color, position: stages.length }); if (error) throw new Error(error.message); setNewStage(""); invalidate(["stages"]); toast.success("Etapa adicionada."); } catch (e) { toast.error(e instanceof Error ? e.message : "Erro ao criar etapa."); } }
  ...
  async function removeStage(id: string) { if (prospects.some((p) => p.stage_id === id)) return void toast.error("Mova os prospects desta etapa antes de excluí-la."); const { error } = await supabase.from("pipeline_stages").delete().eq("id", id); if (error) toast.error("Não foi possível excluir a etapa."); else { invalidate(["stages"]); toast.success("Etapa excluída."); } }
  async function reorder(id: string, direction: -1 | 1) { const index = stages.findIndex((s) => s.id === id); const target = stages[index + direction]; const current = stages[index]; if (!target || !current) return; const results = await Promise.all([supabase.from("pipeline_stages").update({ position: target.position }).eq("id", current.id), supabase.from("pipeline_stages").update({ position: current.position }).eq("id", target.id)]); if (results.some((result) => result.error)) return void toast.error("Não foi possível reordenar as etapas."); invalidate(["stages"]); }
  ...
  return <div className="space-y-6">
    ...
    {!stages.length && <div className="rounded-2xl border border-dashed border-border p-10 text-center text-sm text-muted-foreground">Nenhuma etapa disponível. Crie a primeira etapa acima.</div>}
    <div className="flex gap-4 overflow-x-auto pb-5">
      {stages.map((stage, index) => { const cards = prospects.filter((p) => p.stage_id === stage.id); return <section key={stage.id} onDragOver={(e) => { e.preventDefault(); setOverStage(stage.id); }} onDragLeave={() => setOverStage((s) => s === stage.id ? null : s)} onDrop={(e) => { e.preventDefault(); if (dragId) void moveProspect(dragId, stage.id); setDragId(null); setOverStage(null); }} className={cn("flex w-80 min-h-[32rem] shrink-0 flex-col rounded-2xl border border-border/80 bg-card/65 p-3 shadow-sm backdrop-blur-sm transition-all", overStage === stage.id && "border-primary/50 bg-primary/[.035] shadow-lg shadow-primary/5")}>
