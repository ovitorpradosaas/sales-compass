...
export function ProspectSheet({ prospect, onOpenChange }: ProspectSheetProps) {
  ...
  const save = async () => {
    ...
    if (form.stage_id !== prospect.stage_id) {
      const nextStage = stages.find((s) => s.id === form.stage_id);
      changes.push({ type: "stage_alterada", description: `Etapa alterada para ${nextStage?.name ?? "sem etapa"}.` });
    }
    if (form.status !== prospect.status) changes.push({ type: "status_alterado", description: `Status alterado para ${form.status}.` });
    if (form.next_action !== prospect.next_action || String(form.next_action_at ?? "") !== String(prospect.next_action_at ?? "")) {
      changes.push({ type: "proxima_acao_alterada", description: form.next_action ? `Próxima ação: ${form.next_action}.` : "Próxima ação removida." });
    }
    if (form.contact_name !== prospect.contact_name || form.contact_role !== prospect.contact_role || form.whatsapp !== prospect.whatsapp || form.phone !== prospect.phone || form.email !== prospect.email) {
      changes.push({ type: "contato_atualizado", description: "Dados de contato atualizados." });
    }
    if (changes.length) {
      ...
      <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
        ...
        <div className="space-y-6 px-4 pb-8">
          ...
          <section className="space-y-3"><h3 className="text-sm font-semibold">Contato</h3><div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <Field label="WhatsApp" value={form.whatsapp ?? ""} onChange={(v) => set("whatsapp", v)} /><Field label="Telefone" value={form.phone ?? ""} onChange={(v) => set("phone", v)} />
          ...
          <section className="space-y-3"><h3 className="text-sm font-semibold">Qualificação e pipeline</h3><div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-1.5"><Label>Tags</Label><div className="flex flex-wrap gap-2">{tags.map((t) => { const active = prospectTagIds.includes(t.id); return <button key={t.id} type="button" onClick={() => void toggleTag(t.id, active)} className="focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"><Badge variant={active ? "default" : "outline"}>{t.name}</Badge></button>; })}</div></div>
          ...
          <div className="flex gap-2"><Button onClick={() => void save()} disabled={saving}>{saving ? "Salvando..." : "Salvar ficha"}</Button><Button asChild variant="outline"><Link to="/whatsapp">Abrir conversa</Link></Button></div>
          <Separator />
